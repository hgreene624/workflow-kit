...
description: >
  ...
  wants feedback before planning. This skill focuses on review only — use /create-note PL afterward
...
2. **`/create-note PL`** — produce plan document + Plane project (run after review issues are resolved)
...
> 2. If the project involves VPS infrastructure (check `CLAUDE.md` for VPS paths), SSH in and check:
...
4. **Enforce the quality gate:**
   - **PASS** (0 Critical, score >= 8): "Spec is ready for `/create-note PL`."
   - **CONDITIONAL** (0 Critical, score 5-7): "Spec can proceed to `/create-note PL` but has warnings worth addressing. Proceed anyway?"
   - **FAIL** (any Critical issues OR score < 5): "Spec is not ready for planning. {N} Critical issues must be resolved first." Do NOT suggest proceeding to `/create-note PL`.

## What This Skill Does NOT Do

- It does not produce a plan document — that's `/create-note PL`
- It does not create Plane projects or issues — that's `/create-note PL`
...
## Handoff to /create-note PL

When the user is ready to plan, they run:
```
/create-note PL <path-to-spec>
```

The create-note PL skill will automatically look for review artifacts in `Reviews/{date}/` and use them to inform the plan. Clarification answers from the log directly shape phase structure, task dependencies, and risk mitigations in the plan.
