...
- `<SIGNAL_ENGINE>` / `{{API_CONTAINER}}` (backend, usually independent)
...
**For {{ORG}} projects:** All new work targets `<YOUR_ORG>/<YOUR_MONOREPO>` (the monorepo). Old individual repos (`<APP_1>`, `<APP_2>`, `<YOUR_ADMIN_APP>`, `flora-work-intelligence`, etc.) are **read-only for new features**. They may receive hotfixes only.
